...
사용법: `./scripts/run-test.sh <baseline|localdns> <qps> [runs=3] [nodes=5]`
...
for qps in 20 40 80 160; do
  ./scripts/run-test.sh baseline $qps 3 5
...
각 조건(노드 수 × QPS × LocalDNS 유무)에 대해 3회 시행하고, 시행별로 다음 지표를 독립적으로 집계한다.
